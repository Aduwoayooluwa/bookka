...
const features = [
  { 
    ...
    description: 'Create and dispatch professional invoices with ease and speed.', 
    icon: '/icons/invoice_icon.png' 
  },
  { 
    title: 'Effortless Expense Management', 
    description: 'Track all business expenses with precision for optimal financial control.', 
    icon: '/icons/bookkeeping_icon.png' 
  },
  { 
    title: 'Comprehensive Financial Insights', 
    description: 'Gain valuable insights through detailed, user-friendly financial reports.', 
          ...
          <dl className="space-y-10 md:space-y-0 md:grid md:grid-cols-3 md:gap-x-6 md:gap-y-10">
            {features.map((feature, index) => (
             <div className="flex flex-col" key={index}>
              ...
              >
                <dt className="" >
                  <h3 className="text-center text-2xl mt-16 font-medium text-gray-900">{feature.title}</h3>
                </dt>
                <dd className="mt-6 text-lg text-center text-gray-500 ">{feature.description}</dd>
              </motion.div>
             </div>
            ))}
          </dl>
